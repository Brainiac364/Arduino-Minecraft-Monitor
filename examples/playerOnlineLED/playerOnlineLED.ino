...
/*
  ...
  data from the server.  It will check if the player you specify is online and turn on
  an LED if they are.
...
//Define the username of the player we are looking for
String targetPlayer = "Notch";

//Set aside a string that will contain all of the online players
...
void loop() {
  ...
  if (mc.getStats()) { //If we are able to connect to the server:
    ...
    onlinePlayers = mc.getPlayers();
    Serial.print("Online Player Names: ");
    Serial.println(onlinePlayers);

    // Search for the username of our friend in this list
    if(onlinePlayers.indexOf(targetPlayer) >= 0) {// If we find it
